...
Objective: Persistent event stream listener for ZWO Seestar port 4700.
           ...
           Reconnects automatically after 2s on disconnect (telescope closes
           connections every ~15s by design on firmware 7.18).

           Architecture note (firmware 7.18):
           Port 4700 is a publish/subscribe event bus. The phone app holds
           the session lock and receives command responses. SeeVar listens
           as a passive observer and derives full hardware state from the
           broadcast event stream. No commands are sent by this module.
           ...
           Event vocabulary (observed on S30-Pro firmware 7.18):
             ...
             ViewState     -> (TBD - capture on first light)
"""
...
RECONNECT_DELAY = 2      # seconds — telescope closes connections every ~15s
RECV_TIMEOUT    = 5.0    # socket read timeout
SILENCE_LIMIT   = 60     # seconds of silence = assume disconnect
                ...
                try:
                    ...
                    while b"\r\n" in buf:
                        ...
                        try:
                            ...
                            write_state(state)
                            log.debug("[%s] %s", event, msg)

                            # Log new event types at INFO for vocabulary discovery
...
if __name__ == "__main__":
    ...
    log.info("WilhelminaMonitor v1.0.1 starting — target: %s:%d", host, CTRL_PORT)
